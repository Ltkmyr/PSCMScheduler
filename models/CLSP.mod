...
#            Germany
# Model:     CLSP - Capacitated Lotsizing Problem
##############################################################

param PATH symbolic := "out/CLSP.chart";  # output directory (relative path to app directory)
param EPS := 1e-04;
...
subject to Variable_UB {k in K, t in 1..T}:
  q[k,t] - min((b[t]-tr[k])/tb[k],sum{j in t..T}d[k,j])  * gamma[k,t] <= 0;
